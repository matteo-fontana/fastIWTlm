...
IWTimage = function (IWT_result, alpha = 0.05, abscissa_range = c(0,1),
                     nlevel = 20, plot_unadjusted = F) {
    ...
    if (class(IWT_result)=='IWT1' | class(IWT_result)=='IWT2') {
        p = length(IWT_result$corre)
        
        # Heatmap matrix
        heatmap_matrix = matrix(nrow = p, ncol = 4*p)
        for(i in 0 : (p-1)) {
            for(j in 1 : (2*p)) {
                heatmap_matrix[p-i,j+i+p] = IWT_result$inter[p-i,(j+1)%/%2]
                if(j+i>2*p-i){
                    heatmap_matrix[p-i,j+i-p] = IWT_result$inter[p-i,(j+1)%/%2]
                }
            }
        ...
        ordinata_grafico = seq(abscissa_range[1], abscissa_range[2], length.out = p) -
                           abscissa_range[1]
        ...
        # 1) Heatmap
        par(mar = c(4.1, 4.1, 3, 0.2), cex.main = 1.5, cex.lab = 1.1, las = 0)
        ...
        # 3) Adjusted p-values
        # par(mar = c(4.1, 4.1, 3, 0.2), las = 0) # margins too large!
        par(mar = c(2.1, 4.1, 3, 0.2), las = 0)
        abscissa_pval = seq(abscissa_range[1], abscissa_range[2], length.out = p)
        plot(abscissa_pval, IWT_result$corre, ylim = c(0,1), 
             xlim = c(min_plot, max_plot), main='Adjusted p-value function', 
             ylab = 'p-value', xlab = 'Abscissa', xaxs = 'i',type = 'l', lwd = 2, lty = 1)
        if (plot_unadjusted==T) {
            lines(abscissa_pval, IWT_result$point, lwd = 2, lty = 2)
        }
        difference = which(IWT_result$corre < alpha)
        ...
        lines(abscissa_pval, IWT_result$corre, pch = 16, lwd = 2, lty = 1)
        if (plot_unadjusted==T)
            lines(abscissa_pval, IWT_result$point, lwd = 2, lty = 2)
        
        # 4) Functional data
        if (IWT_result$test == '2pop') col_pop = IWT_result$ord_labels
        else col_pop = 'black'
        matplot(abscissa_pval, t(IWT_result$data.eval), col = col_pop, type = 'l', 
        ...
        matplot(abscissa_pval, t(IWT_result$data.eval), col = col_pop, type = 'l', add = T)
        if (IWT_result$test == '1pop'){
            if (length(IWT_result$mu) == 1) mu_eval = rep(IWT_result$mu, p)
        ...
        for (i in 0 : (p-1)) {
            for (j in 1 : (2*p)) {
                ...
                for (ii in 1 : nvar) {
                    heatmap_matrix_factor[ii,p-i,j+i+p] =
                        IWT_result$pval_matrix_factor[ii,p-i,(j+1)%/%2]
                    if (j+i > 2*p-i)
                        heatmap_matrix_factor[ii,p-i,j+i-p] =
                            IWT_result$pval_matrix_factor[ii,p-i,(j+1)%/%2]
                }
        ...
        ordinata_grafico = seq(abscissa_range[1], abscissa_range[2],
        ...
        for (j in 0 : 10) abline(h = j / 10, col = 'lightgray', lty = 'dotted')
        ...
        for(var in 1 : nvar) {
            ...
            image(ascissa_quad, ordinata_grafico,t(matrice_quad_factor[p:1,]), 
                  col = colori, ylab = 'Interval length',
            ...
            if (length(difference) > 0) {
                for (j in 1 : length(difference)) {
                    min_rect = abscissa_pval[difference[j]] - (abscissa_pval[2] -
                                                               abscissa_pval[1])/2
                    max_rect = min_rect + (abscissa_pval[2] - abscissa_pval[1])
                    rect(min_rect, par('usr')[3], max_rect, par('usr')[4], 
                         col = 'gray90', density = -2, border = NA)
            ...
            for (j in 0 : 10) abline(h = j / 10, col = 'lightgray', lty = 'dotted')
            lines(abscissa_pval, IWT_result$adjusted_pval_factor[var,],
                  pch = 16, lwd = 2, lty = 1)
            if(plot_unadjusted==T)
                lines(abscissa_pval, IWT_result$unadjusted_pval_factor[var,], lwd = 2, lty = 2)
            ...
            if (length(grep(':', var_name)) > 0) { # Plot interaction
                ...
                if (length(dim(colors)) > 1) colors = (apply(colors, 1, paste, collapse = ''))
            ...
            matplot(abscissa_pval, t(IWT_result$data.eval), col = colors, type = 'l', 
                    main = 'Functional data', xlab = 'Abscissa', 
                    ylab = 'Value', xaxs = 'i', xlim = c(min_plot, max_plot))
            if (length(difference) > 0) {
                for (j in 1 : length(difference)) {
            ...
            matplot(abscissa_pval, t(IWT_result$data.eval), col = colors,type = 'l', add = T)
        }
    }
}
